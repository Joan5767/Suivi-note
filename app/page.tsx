        ...
        <div className="fixed inset-0 bg-black/35 z-[12500] flex items-center justify-center p-4 backdrop-blur-[2px]" onClick={() => setShowPlanningGestures(false)}>
          <div className="w-full max-w-sm rounded-[28px] bg-[#FBF9F4] border border-[#DDD5C7] shadow-2xl p-5 text-[#4A463F]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm font-semibold text-[#655E54]">
              ...
              <p>📌 Les jours restent visibles en haut pendant le défilement.</p>
            </div>
          </div>
        </div>
      )}
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
               {savedTemplates.map((tmpl, index) => (
                 ...
                 >
                   ...
                   {/* Boutons d'action */}
                   <div className="p-2 flex items-center gap-1.5 bg-[#F4F0E9]">
                     <button onClick={() => setPreviewTemplate(tmpl)} className="px-2.5 py-1.5 bg-white hover:bg-[#ECE7DE] text-[#665F55] border border-[#DDD5C9] font-bold text-[10px] rounded-lg transition-colors">Aperçu</button>
                     <button onClick={() => loadTemplate(tmpl)} className="flex-1 bg-[#C8D2BC] hover:bg-[#BAC7AD] text-[#35412F] font-black text-[10px] py-1.5 px-2 rounded-lg transition-colors">Charger</button>
                     <button onClick={() => duplicateSavedTemplate(tmpl)} className="px-2 py-1.5 bg-[#E2D6C7] hover:bg-[#D7C7B5] text-[#59493B] font-black text-[10px] rounded-lg transition-colors" aria-label="Dupliquer">⧉</button>
                     <button onClick={() => deleteSavedTemplate(tmpl.id)} className="px-2 py-1.5 bg-[#F0DDD7] hover:bg-[#E8CEC6] text-[#885C50] font-black text-[10px] rounded-lg transition-colors" aria-label="Supprimer">🗑</button>
                   </div>
                 </div>
               ))}
             </div>
