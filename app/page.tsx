...
function DrawNoteEditor({ initialData, initialTitle, initialColor, initialPinned, canPin, onSave, onDelete, registerAutoSave, onClose }: DrawEditorProps) {
  ...
  } as Record<MemoColor, string>)[memoColor];
            ...
            {(['sage', 'sand', 'rose', 'blue', 'lavender', 'white'] as MemoColor[]).map(color => (
              <button
                ...
                className={`w-7 h-7 rounded-full border-2 transition-transform ${({
                  sage: 'bg-[#E6ECDD] border-[#CAD5BE]',
                  sand: 'bg-[#F1E4D2] border-[#DEC9AD]',
                  rose: 'bg-[#F2DEDA] border-[#DEC0B9]',
                  blue: 'bg-[#DFE8EC] border-[#C2D3DA]',
                  lavender: 'bg-[#E9E2EF] border-[#D3C6DE]',
                  white: 'bg-[#FBFAF7] border-[#DED7CC]',
                } as Record<MemoColor, string>)[color]} ${memoColor === color ? 'scale-110 ring-2 ring-black/20 ring-offset-1' : 'opacity-80'}`}
              />
            ))}
          </div>
...
export default function Home() {
  ...
  const normalizeMemoEntry = (row: any): MemoEntry => {
    ...
    return {
      ...
      // L'épinglage n'existe que dans les notes actives.
      pinned: archived ? false : Boolean(row?.pinned),
      archived,
  ...
  const moveMemoLongPress = (e: React.PointerEvent<HTMLElement>) => {
    ...
    // Un mouvement horizontal commencé avant l'appui long appartient au
    // balayage Actives/Archives, pas au déplacement de la carte. Un véritable
    // appui long a déjà activé drag.active avant d'arriver ici.
    ...
    return (
      ...
      >
        ...
        <div className="flex items-start gap-2">
          ...
          {!showMemoArchived && memo.pinned && (
            <button
              type="button"
              onPointerDown={(event) => event.stopPropagation()}
              onClick={(event) => {
                event.stopPropagation();
                void updateMemo(memo.id, { pinned: false });
              }}
              className="w-7 h-7 -mt-1 -mr-1 rounded-full flex items-center justify-center text-xs flex-shrink-0 hover:bg-white/40 active:scale-90"
              aria-label="Désépingler cette note"
              title="Désépingler"
            >📌</button>
          )}
        </div>
              ...
              return (
                <div className={`pointer-events-none rounded-[18px] border p-3 shadow-2xl scale-[1.025] ${memoColorClasses(memo.color)}`} style={{ width: 'min(46vw, 380px)' }}>
                  <div className="flex items-start gap-2">
                    ...
                    {!showMemoArchived && memo.pinned && <span className="text-xs flex-shrink-0">📌</span>}
                  </div>
                  {memo.is_drawing && (
                    <DrawingPreview data={memo.drawing_data} className="mt-2.5 max-h-[190px]" />
                  )}
